...
class SingleContainerManager:
    ...
    def start_single_container(self, model_key: str) -> Tuple[bool, str]:
        ...
        if current_active == model_key:
            ...
            if api_healthy:
                return (
                    True,
                    f"Модель {config['display_name']} уже активна и готова к работе",
                )
        ...
        try:
            ...
            st.info(
                f"⏳ Ожидание готовности модели (до {config['startup_time']} сек)..."
            )
            ...
            while time.time() - start_time < max_wait:
                ...
                if container_status["running"]:
                    # Проверяем API
                    api_healthy, api_message = self.check_api_health(
                        config["port"], timeout=3
                    )

                    if api_healthy:
                        progress_bar.progress(1.0)
                        status_text.success(
                            f"✅ {config['display_name']} готов к работе!"
                        )
                        self.current_active_model = model_key
                    ...
                    status_text.info(
                        f"🔄 Запуск контейнера... ({container_status['status']})"
                    )
            ...
            return (
                False,
                f"Таймаут запуска модели {config['display_name']} ({max_wait} сек)",
            )
    ...
    def _build_docker_command(
        ...
        cmd.extend(
            [
                "-v",
                f"{cache_path}:/root/.cache/huggingface:rw",
                "-e",
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_model_name": (
                self.models_config[active_model]["display_name"]
                if active_model
                else None
            ),
    ...
    def create_model_selector_ui(self) -> Optional[str]:
        ...
        st.info(
            "💡 **Принцип работы:** Только одна модель активна одновременно для экономии GPU памяти"
        )
        ...
        for model_key, model_status in status["models"].items():
            ...
            option_text = (
                f"{status_icon} {config['display_name']} ({config['memory_gb']} ГБ)"
            )
        ...
        if selected_model_key != status["active_model"]:
            if st.button(
                f"🔄 Переключиться на {selected_config['display_name']}", type="primary"
            ):
    ...
    def create_status_dashboard(self) -> None:
        ...
        for model_key, model_status in status["models"].items():
            ...
            with st.expander(
                f"{status_color} {config['display_name']} - {status_text}"
            ):
