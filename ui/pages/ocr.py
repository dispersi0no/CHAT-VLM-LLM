...
def clean_ocr_result(text: str) -> str:
    ...
    text = re.sub(
        r"4a\)(\d{2}\.\d{2}\.\d{4})4b\)(\d{2}\.\d{2}\.\d{4})", r"4a) \1 4b) \2", text
    )
...
def show_ocr(config: dict, execution_mode: str, selected_model: str) -> None:
    ...
    with col1:
        ...
        if uploaded_file:
            ...
            st.caption(
                f"📐 Размер: {image.size[0]}x{image.size[1]} | Формат: {image.format}"
            )
            ...
            try:
                ...
                if active_model:
                    ...
                    if "dots" in active_config["model_path"].lower():
                        render_alert(
                            "✅ Специализированная OCR модель - отличный выбор для извлечения текста!",
                            "info",
                        )
                    else:
                        render_alert(
                    ...
                    render_alert(
                        "⚠️ Нет активной модели. Будет активирована специализированная dots.ocr",
                        "warning",
                    )
                    ...
                    try:
                        ...
                        if enhance_image or denoise or deskew:
                            ...
                            if max(processed_image.size) > max_size:
                                ...
                                new_size = tuple(
                                    int(dim * ratio) for dim in processed_image.size
                                )
                        ...
                        if "vLLM" in execution_mode:
                            try:
                                ...
                                if "vllm_adapter" not in st.session_state:
                                    st.session_state.vllm_adapter = (
                                        VLLMStreamlitAdapter()
                                    )

                                if st.button(
                                    "🔄 Обновить статус моделей",
                                    key="refresh_adapter_ocr",
                                ...
                                active_model = (
                                    adapter.container_manager.get_active_model()
                                )
                                ...
                                if not active_model:
                                    for (
                                        ...
                                        adapter.container_manager.models_config.items()
                                    ):
                                        ...
                                        if container_status["running"]:
                                            ...
                                            st.info(
                                                f"🔄 Обнаружена загружающаяся модель: {model_cfg['display_name']}"
                                            )
                                            break

                                if active_model:
                                    active_config = (
                                        adapter.container_manager.models_config[
                                            active_model
                                        ]
                                    )
                                    vllm_model = active_config["model_path"]

                                    api_healthy, api_message = (
                                        adapter.container_manager.check_api_health(
                                            active_config["port"]
                                        )
                                    )
                                    ...
                                    if api_healthy:
                                        st.success(
                                            f"🎯 Используем готовую модель: {active_config['display_name']}"
                                        )
                                    else:
                                        st.info(
                                            f"⏳ Ожидаем готовности модели: {active_config['display_name']} ({api_message})"
                                        )
                                        render_alert(
                                ...
                                result = adapter.process_image(
                                    processed_image, prompt, vllm_model, max_tokens
                                )
                                ...
                                if result and result["success"]:
                                    ...
                                    st.success(
                                        f"✅ Обработано через vLLM за {processing_time:.1f} сек"
                                    )
                                else:
                                ...
                                render_alert(
                                    "💡 Переключаемся на Transformers режим...", "info"
                                )
                                ...
                                if hasattr(model, "extract_text"):
                                    ...
                                    text = model.chat(
                                        processed_image,
                                        "Извлеките весь текст из этого документа, сохраняя структуру и форматирование.",
                                    )
                        else:
                            model = ModelLoader.load_model(selected_model)
                        ...
                        if len([word for word in text.split() if len(word) > 2]) > 5:
                            quality_score += 0.1
                        if any(
                            date_pattern in text
                            for date_pattern in [r"\d{2}\.\d{2}\.\d{4}", r"\d{4}"]
                        ):
                            quality_score += 0.05
                        if any(
                            field in text
                            for field in ["1.", "2.", "3.", "4a)", "4b)", "4c)", "5."]
                        ):
                            quality_score += 0.05
