...
def extract_text(image_path, model="qwen3_vl_2b", language=None):
    ...
    with open(image_path, "rb") as f:
        ...
        response = requests.post(
            f"{API_URL}/ocr", files={"file": f}, params=params, timeout=30
        )
...
def batch_ocr(image_paths, model="qwen3_vl_2b"):
    ...
    try:
        ...
        response = requests.post(
            f"{API_URL}/batch/ocr", files=files, params={"model": model}, timeout=30
        )
    finally:
...
if __name__ == "__main__":
    ...
    if Path(image_path).exists():
        ...
        # 6. Compare models
        comparison = compare_models(image_path, models=["qwen3_vl_2b", "dots_ocr"])
    else:
